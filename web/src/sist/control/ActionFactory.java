...
import sist.action.DeleteMemberAction;
...
public class ActionFactory {
	...
	public static Action getAction(String cmd) {
		...
		if (cmd.equals("login")) {
			...
			action = new StartAction("end.jsp", false); // false하면 dispatcher로
														// 간다.
			...
			action = new MyPageAction("member/identity01.jsp", true);
		}else if (cmd.equals("memberModify")){
			action = new MemberModifyAction("member/identity01_01.jsp", true);
		}else if(cmd.equals("pwChange")){
		    action = new PwChangeAction("member/identity01_02.jsp", true);	
		}else if(cmd.equals("deleteMember")){
			action = new DeleteMemberAction("member/identity02.jsp", true);
		}
